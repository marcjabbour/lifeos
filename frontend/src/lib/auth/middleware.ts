...
async function validateSession(request: NextRequest): Promise<AuthResponse> {
  try {
    // Get auth headers from request to pass through to Supabase
    const authHeader = request.headers.get("authorization");
    const cookieHeader = request.headers.get("cookie");

    // If no auth info at all, return early
    if (!authHeader && !cookieHeader) {
      return { success: false, error: "No session found", status: 401 };
    }

    // Create a Supabase client that passes through the cookies/auth headers
