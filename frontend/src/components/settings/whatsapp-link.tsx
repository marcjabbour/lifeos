...
export function WhatsAppLink() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      const data = await api.get<LinkStatus>("/api/whatsapp/link");
      setStatus(data);
    } catch (err) {
      console.error("WhatsApp status fetch error:", err);
      const apiError = err as { error?: string };
      setError(apiError.error ?? "Failed to fetch WhatsApp status");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const generateCode = async () => {
    ...
    try {
      const data = await api.post<{ code: string; expires_at: string }>(
        "/api/whatsapp/link",
      );
      setStatus({
        linked: false,
      ...
      const apiError = err as { error?: string };
      setError(apiError.error ?? "Failed to generate code");
    } finally {
      setGenerating(false);
    }
  };
  ...
  const unlinkAccount = async () => {
    ...
    try {
      await api.delete("/api/whatsapp/link");
