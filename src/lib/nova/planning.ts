 ...
 */
export function decideAction(reasoning: ReasoningResult, contentSummary: string): DecisionResult {
  ...
  switch (confidenceLevel) {
    ...
    case 'MEDIUM':
      ...
      const actionDesc = suggestedAction?.action.replace(/_/g, ' ') || 'help with this'
      return {
        shouldProceed: false,
        confidenceLevel,
...
export function validatePlan(plan: JobPlan): PlanValidation {
  ...
  if (plan.estimatedTokens > PER_JOB_BUDGET) {
    errors.push(`Estimated tokens (${plan.estimatedTokens}) exceeds budget (${PER_JOB_BUDGET})`)
  }
