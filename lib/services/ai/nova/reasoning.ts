...
import { assembleContext, type ContextAssembly } from "@/lib/services/ai/rag/context";
import type {
  PerceptionResult,
  ReasoningResult,
  LLMResponse,
...
export async function reason(
  input: ReasonInput,
): Promise<LLMResponse<ReasonOutput>> {
...
export function combineReasoningOutputs(outputs: ReasonOutput[]): ReasonOutput {
  ...
  const minConfidence = Math.min(...outputs.map((o) => o.confidence ?? 0));
...
export function filterValidActions(output: ReasonOutput): ReasonOutput {
  return {
    ...output,
    suggestedActions: (output.suggestedActions ?? []).filter((a) =>
