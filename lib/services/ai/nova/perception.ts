...
export async function perceive(
  input: PerceiveInput,
): Promise<LLMResponse<PerceiveOutput>> {
  ...
  const output: PerceiveOutput = {
    ...
    sentiment: response.result.metadata
      ?.sentiment as PerceiveOutput["sentiment"],
    // Extract ambiguity info if present
    ambiguity: response.result.ambiguity as AmbiguityInfo | undefined,
  };
...
function extractUrlInfo(url: string): {
  ...
  try {
    ...
    const platform = Object.keys(platformActions).find((p) =>
      domain.includes(p),
    );
